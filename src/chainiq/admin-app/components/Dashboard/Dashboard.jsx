...
const Dashboard = ({ offices = [], teams = [], staffUsers = [] }) => {
  ...
  const stats = useMemo(() => {
    ...
    const pendingKyc = leads.filter((l) => l.kycStatus === 'Pending').length;
    const approvedKyc = leads.filter((l) => (l.kycStatus || '').toLowerCase() === 'approved').length;
    const ftd = leads.filter((l) => l.stage === 'Deposit' || l.leadStatus === 'Deposit').length;
    const failed = leads.filter((l) => l.stage === 'Failed Deposit' || l.leadStatus === 'Failed Deposit').length;
    const conversion = total ? ((ftd / total) * 100).toFixed(1) : '0.0';
    ...
    return {
      total, online, pendingKyc, approvedKyc, ftd, failed, conversion,
      agents, teamLeaders, officeManagers,
  ...
  return (
    <div id="analytics-section" className="aax-admin-section">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 aax-mb-6">
        ...
        <KpiCard label="First-Time Deposits" value={fmt(stats.ftd)} sub={`${stats.conversion}% conversion`} accent="#30D158" icon="[money]" />
        <KpiCard label="Pending KYC" value={fmt(stats.pendingKyc)} sub={`${fmt(stats.approvedKyc)} approved`} accent="#0A84FF" icon="🛡" />
        <KpiCard label="Failed Deposits" value={fmt(stats.failed)} sub="Requires follow-up" accent="#FF6464" icon="Warning" />
      </div>
